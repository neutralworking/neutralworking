---
title: Daley Blind
category: players
tags: [CD, Girona, Netherlands, Destroyer]
updated: 2026-04-18
source: compiled
confidence: high
summary: CD at Girona. Archetype: Destroyer.
person_id: 9844
backlinks: [destroyer, girona]
---

# Daley Blind

## Overview

| Field | Value |
|---|---|
| **Position** | CD |
| **Club** | Girona |
| **Nation** | Netherlands |
| **Age** | 36 |
| **Archetype** | [[destroyer]] Destroyer |
| **Level** | 83 |
| **Overall** | 73.0 |
| **Pursuit** | — |
| **Market Value Tier** | 3 |

## Snapshot

Composed left-sided defender who brings experience and excellent positional sense; he's comfortable on the ball and can step into midfield to probe. His physical decline is real—quicker attackers run past him regularly, and he struggles with sustained pressure down the line. Useful veteran for a side wanting calm, intelligent defending, but you need cover for his athletic limitations.

## Canon Context

- [[build-up]] Build-Up: first-phase circulation and release security are part of the role context.
- [[rest-defense]] Rest Defense: structural protection behind the ball matters when projecting this profile.
- [[build-up-vs-progression]] Build-Up vs Progression: separating first-phase security from true advancement sharpens the fit call.
- [[overload]] Overload: the profile benefits when the team can create local numerical superiority around the ball.

## Role Fit

- Best-fit role is `Sweeper` with model support score `68`.
- Primary archetype leans toward `Destroyer` behaviour from a `CD` base.
- Tactical fit is strongest when the game asks for Build Up Contributor and Positional Discipline.
- Most bankable strengths show up in Crossing, First Touch, and Heading.

## In Possession

- Ball-value indicators lean on Crossing, First Touch, and Heading.
- Style traits point toward Elegant, Long Range Passing, and Tackling Ability in possession.

## Out of Possession

- Defensive or transition evidence is strongest in Build Up Contributor and Positional Discipline.

## Athletic Profile

- Physical indicators currently centre on Aerial Threat.

## Scouting Indicators

| Indicator | Value | Interpretation |
|---|---|---|
| Crossing | █████████████████ 17/10 | One of the stronger current indicators for this profile at 17/10. |
| First Touch | ████████████████ 16/10 | One of the stronger current indicators for this profile at 16/10. |
| Heading | ████████████████ 16/10 | One of the stronger current indicators for this profile at 16/10. |
| Reactions | ████████████████ 16/10 | One of the stronger current indicators for this profile at 16/10. |
| Long Range | ███████████████ 15/10 | One of the stronger current indicators for this profile at 15/10. |
| Physical | ███████████████ 15/10 | One of the stronger current indicators for this profile at 15/10. |
| Balance | ██████████████ 14/10 | One of the stronger current indicators for this profile at 14/10. |
| Jumping | ██████████████ 14/10 | One of the stronger current indicators for this profile at 14/10. |

## Valuation Range

| Layer | Estimate |
|---|---|
| Floor | EUR 0.8m |
| Mid | EUR 1.0m |
| Ceiling | EUR 1.5m |
| Confidence | High |

- Value drivers: Chief Scout valuation anchor, role scarcity, and acquisition context.
- Discount factors: Negotiation premium, contract leverage, and league-to-league portability.
- Market backdrop: [[transfer-market-2025]] frames the current inflation and fee-pressure environment.

## Risk Flags

- No major automated red flag surfaced from the current data slice.

## Personality

- **Game Reading (EI)**: 66
- **Motivation (SN)**: 35
- **Social (TF)**: 40
- **Pressure (JP)**: 31
- **Competitiveness**: 6/10
- **Coachability**: 5/10

## Trait Ledger

- **Aerial Threat** (physical) — severity 10/10
- **Build Up Contributor** (tactical) — severity 9/10
- **Elegant** (style) — severity 8/10
- **Quiet Leader** (behavioral) — severity 8/10
- **Positional Discipline** (tactical) — severity 6/10
- **Long Range Passing** (style) — severity 4/10
- **Tackling Ability** (style) — severity 4/10
- **Defensive Awareness** (style) — severity 4/10
- **Passing Ability** (style) — severity 4/10

## Evidence Ledger

| Evidence Type | Source | Date | Strength | Note |
|---|---|---|---|---|
| Attribute model | Chief Scout DB | 2026-04-18 | High | 46 graded indicators available. |
| Trait model | Chief Scout DB | 2026-04-18 | High | 9 tagged traits linked to style or tactical behaviour. |
| Personality model | Chief Scout DB | 2026-04-18 | Medium | Psychological profile available for projection context. |
| Scouting note | docs/Scouting or player_status | 2026-04-18 | Medium | Human-authored note incorporated into summary. |
