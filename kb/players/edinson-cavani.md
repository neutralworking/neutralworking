---
title: Edinson Cavani
category: players
tags: [CF, Boca Juniors, Uruguay, Striker]
updated: 2026-04-19
source: compiled
confidence: high
summary: CF at Boca Juniors. Archetype: Striker.
person_id: 10485
backlinks: [striker, target-forward, boca-juniors]
---

# Edinson Cavani

## Overview

| Field | Value |
|---|---|
| **Position** | CF |
| **Club** | Boca Juniors |
| **Nation** | Uruguay |
| **Age** | 39 |
| **Archetype** | [[striker]] Striker |
| **Best Role** | [[target-forward]] Target Forward |
| **Role Score** | 43 |
| **Level** | 44 |
| **Overall** | 37.0 |
| **Pursuit** | — |
| **Market Value Tier** | 1 |

## Snapshot

A late-career resurrection that's genuinely moving to watch—the man still understands positioning and movement in tight spaces with the clarity of someone who's terrorized defences for two decades. His pressing remains intelligent, he still finds pockets in the box, and his heading is a genuine threat from set pieces. But the acceleration is gone, the explosive first step that made him lethal on the turn has withered, and he cannot drive a match when it demands mobility or sustained intensity; at 39, he's a finisher, not a creator, and teams need to service him constantly or he becomes a passenger.

## Canon Context

- [[half-space]] Half-Space: interior receiving lanes and body orientation influence the player's best usage.
- [[third-man-run]] Third-Man Run: combination play and timing beyond the first receiver often shape this role.
- [[pinning]] Pinning: stretching the line and fixing defenders helps explain the value of this role.
- [[occupation-of-five-lanes]] Occupation of Five Lanes: width and lane discipline affect how the role creates or receives space.

## Role Fit

- Best-fit role is [[target-forward]] Target Forward with model support score `43`.
- Primary archetype leans toward `Striker` behaviour from a `CF` base.
- Tactical fit is strongest when the game asks for Progressive Carrier.
- Most bankable strengths show up in Aerial Duels, Movement, and Jumping.

## In Possession

- Ball-value indicators lean on Aerial Duels, Movement, and Jumping.
- Style traits point toward Two Footed and Aerial Ability in possession.

## Out of Possession

- Defensive or transition evidence is strongest in Progressive Carrier.

## Athletic Profile

- Physical indicators currently centre on Availability, Aerial Threat, and Durability.

## Scouting Indicators

| Indicator | Value | Interpretation |
|---|---|---|
| Aerial Duels | █████████████████ 17/10 | One of the stronger current indicators for this profile at 17/10. |
| Movement | █████████████████ 17/10 | One of the stronger current indicators for this profile at 17/10. |
| Jumping | ████████████████ 16/10 | One of the stronger current indicators for this profile at 16/10. |
| Reactions | ████████████████ 16/10 | One of the stronger current indicators for this profile at 16/10. |
| Volleys | ████████████████ 16/10 | One of the stronger current indicators for this profile at 16/10. |
| First Touch | ███████████████ 15/10 | One of the stronger current indicators for this profile at 15/10. |
| Physical | ███████████████ 15/10 | One of the stronger current indicators for this profile at 15/10. |
| Crossing | █████████████ 13/10 | One of the stronger current indicators for this profile at 13/10. |

## Valuation Range

| Layer | Estimate |
|---|---|
| Floor | EUR 0.5m |
| Mid | EUR 1.0m |
| Ceiling | EUR 1.2m |
| Confidence | High |

- Value drivers: Chief Scout valuation anchor, role scarcity, and acquisition context.
- Discount factors: Negotiation premium, contract leverage, and league-to-league portability.
- Market backdrop: [[transfer-market-2025]] frames the current inflation and fee-pressure environment.

## Risk Flags

- Coachability score is low enough to raise integration risk in demanding environments.

## Personality

- **Game Reading (EI)**: 42
- **Motivation (SN)**: 46
- **Social (TF)**: 77
- **Pressure (JP)**: 60
- **Competitiveness**: 4/10
- **Coachability**: 3/10

## Trait Ledger

- **Availability** (physical) — severity 10/10
- **Aerial Threat** (physical) — severity 10/10
- **Progressive Carrier** (tactical) — severity 7/10
- **Two Footed** (style) — severity 7/10
- **Aerial Ability** (style) — severity 5/10
- **Durability** (physical) — severity 2/10

## Evidence Ledger

| Evidence Type | Source | Date | Strength | Note |
|---|---|---|---|---|
| Attribute model | Chief Scout DB | 2026-04-19 | High | 45 graded indicators available. |
| Trait model | Chief Scout DB | 2026-04-19 | High | 6 tagged traits linked to style or tactical behaviour. |
| Personality model | Chief Scout DB | 2026-04-19 | Medium | Psychological profile available for projection context. |
| Scouting note | docs/Scouting or player_status | 2026-04-19 | Medium | Human-authored note incorporated into summary. |
