---
title: Sergio Gomez
category: players
tags: [WD, Real Sociedad, Spain, Creator]
updated: 2026-04-17
source: compiled
confidence: high
summary: WD at Real Sociedad. Archetype: Creator.
person_id: 17347
backlinks: [creator, real-sociedad]
---

# Sergio Gomez

## Overview

| Field | Value |
|---|---|
| **Position** | WD |
| **Club** | Real Sociedad |
| **Nation** | Spain |
| **Age** | 44 |
| **Archetype** | [[creator]] Creator |
| **Level** | 84 |
| **Overall** | 78.0 |
| **Pursuit** | None |
| **Market Value Tier** | 3 |

## Snapshot

Left-back/winger with pace and attacking instinct; defensively inconsistent and positionally naive, better suited to attacking than defending. Young player with athleticism but requiring tactical maturation.

## Role Fit

- Best-fit role is `Fullback` with model support score `78`.
- Primary archetype leans toward `Creator` behaviour from a `WD` base.
- Tactical fit is strongest when the game asks for High Press.
- Most bankable strengths show up in Balance, Shielding, and Crossing.

## In Possession

- Ball-value indicators lean on Balance, Shielding, and Crossing.
- Style traits point toward Skill Moves and Trickery in possession.

## Out of Possession

- Defensive or transition evidence is strongest in High Press.

## Athletic Profile

- Physical indicators currently centre on Availability and Durability.

## Scouting Indicators

| Indicator | Value | Interpretation |
|---|---|---|
| Balance | █████████████████ 17/10 | One of the stronger current indicators for this profile at 17/10. |
| Shielding | █████████████████ 17/10 | One of the stronger current indicators for this profile at 17/10. |
| Crossing | ████████████████ 16/10 | One of the stronger current indicators for this profile at 16/10. |
| Penalties | ████████████████ 16/10 | One of the stronger current indicators for this profile at 16/10. |
| Acceleration | ███████████████ 15/10 | One of the stronger current indicators for this profile at 15/10. |
| First Touch | ███████████████ 15/10 | One of the stronger current indicators for this profile at 15/10. |
| Reactions | ███████████████ 15/10 | One of the stronger current indicators for this profile at 15/10. |
| Stamina | ███████████████ 15/10 | One of the stronger current indicators for this profile at 15/10. |

## Valuation Range

| Layer | Estimate |
|---|---|
| Floor | EUR 2.4m |
| Mid | EUR 3.0m |
| Ceiling | EUR 3.6m |
| Confidence | Medium |

- Value drivers: Role scarcity, current level, and available data depth.
- Discount factors: Uncertainty in comparable fees and context portability.

## Risk Flags

- No major automated red flag surfaced from the current data slice.

## Personality

- **Game Reading (EI)**: 53
- **Motivation (SN)**: 42
- **Social (TF)**: 56
- **Pressure (JP)**: 45
- **Competitiveness**: 6/10
- **Coachability**: 4/10

## Trait Ledger

- **Availability** (physical) — severity 10/10
- **Durability** (physical) — severity 6/10
- **High Press** (tactical) — severity 6/10
- **Skill Moves** (style) — severity 6/10
- **Trickery** (style) — severity 5/10

## Evidence Ledger

| Evidence Type | Source | Date | Strength | Note |
|---|---|---|---|---|
| Attribute model | Chief Scout DB | 2026-04-17 | High | 44 graded indicators available. |
| Trait model | Chief Scout DB | 2026-04-17 | High | 5 tagged traits linked to style or tactical behaviour. |
| Personality model | Chief Scout DB | 2026-04-17 | Medium | Psychological profile available for projection context. |
| Scouting note | docs/Scouting or player_status | 2026-04-17 | Medium | Human-authored note incorporated into summary. |
