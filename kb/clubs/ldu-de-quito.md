---
title: LDU de Quito
category: clubs
tags: []
updated: 2026-04-19
source: compiled
confidence: medium
summary: LDU de Quito. 20 players in database.
---

# LDU de Quito

**League**: —  
**Nation**: —  
**Players in DB**: 20

## Identity

- Database coverage is deepest in GK (1) and CF (1), which shapes the visible squad picture.
- The tracked archetype mix leans toward Striker (1) across the current squad.
- Average available overall grade sits at 59.0, so this article should be read as a roster-structure snapshot rather than a hand-written tactical dossier.

## Canon Context

- [[build-up]] Build-Up: the squad shape still depends on how the club exits the first line and distributes responsibility.
- [[rest-defense]] Rest Defense: roster balance behind the ball affects whether the club can attack without exposing transitions.
- [[box-entry]] Box Entry: the attacking profiles should be judged by how reliably the club turns territory into area access.
- [[switch-of-play]] Switch of Play: the roster may be best understood through how it can move defenses before attacking the final action.

## Squad

| Player | Position | Archetype | Overall |
|---|---|---|---|
| [[a-arce]] A. Arce | CF | Striker | 59.0 |
| [[gonzalo-valle]] Gonzalo Valle | GK | — | — |
| Alejandro Jair Cabeza Jiménez | — | — | — |
| Alexander Antonio Alvarado Carriel | — | — | — |
| Alexander Domínguez Carabalí | — | — | — |
| Bryan Josías Ramírez León | — | — | — |
| Daniel Isai de la Cruz de la Cruz | — | — | — |
| Darío Javier Aimar Álvarez | — | — | — |
| Fernando Nicolás Cornejo Miranda | — | — | — |
| Gabriel Alejandro Villamíl Cortéz | — | — | — |
| Gian Franco Allala Menéndez | — | — | — |
| José Alfredo Quintero Ordóñez | — | — | — |
| Kevin Andres Minda Ruales | — | — | — |
| Leonel Enrique Quiñónez Padilla | — | — | — |
| Lisandro Joel Alzugaray | — | — | — |
| Melvin Fabricio Díaz Canencia | — | — | — |
| Michael Steveen Estrada Martínez | — | — | — |
| Ricardo Ade | — | — | — |
| Richard Alexander Mina Caicedo | — | — | — |
| Yeltzin Hernán Erique Villigua | — | — | — |
